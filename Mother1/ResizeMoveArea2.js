		define([ "Mother1/exchanger2.js",
			...
			function(Exchanger,ResizeWidget,MoveCoord,Utils,Declare,Win,On,Evented,Moveable,Mover,Dom,DomGeom,DomConstruct,DomStyle,DomClass,Lang){ 
				...
				//	  : 2- define an event for "mouseenter" to activate moveResizeArea when user enters the area
					...
					//      se o elemento é o currente, e quisermos activa-lo: fazer activate()
					...
					//                 para caracterizar o novo elemento, e depois fazer activate() para o activar 
					...
					constructor:function(xLabel,landingLeft,landingTop,landingWidth,landingHeight,xBorderThickness,xBorderType,xActiveColor){
						...
						this.util=new Utils(); //to use utils functions
						this.element.innerHTML=this.util.makeDivId(this.moveResizeDivId,this.current.l,this.current.t,this.current.w,this.current.h,this.current.borderThickness,null);
						Win.body().appendChild(this.element); 
						this.visibleElement = DomConstruct.create("div"); //cria outro HTML div 
						this.visibleElement.innerHTML=this.util.getInnerHTML("textBox",this.avatarId,this.current.label,this.current.w,this.current.h,this.current.borderThickness,this.current.borderType,this.current.activeColor); //este é o que se vai ver...
					...
					activate:function(){
						...
						if(!this.objResizeWidget){//ResizeWidget is created and its coordinator property is set to resizeMoveArea (this class)
							...
							this.endResizeHandler=this.objResizeWidget.on("endResize",Lang.hitch(this,function(e){
								...
								console.log("-->"+this.z1+" resizeMoveArea.activate() captou EVENT 'endResize' left="+this.position.x+" top="+this.position.y+" width="+this.position.w+" height="+this.position.h);
						...
						this.oExchanger.setPointer(function(){	
							console.log("-->resizeMoveArea.activate /this.oExchanger.setPointer --------SAI POR MOVE------>this is the way out in moveCoord");
