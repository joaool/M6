		define([ "Mother1/exchanger2.js",
			...
			function(Exchanger,Dbg,ResizeWidget,MoveCoord,Utils,Declare,Win,On,Evented,Moveable,Mover,Dom,DomGeom,DomConstruct,DomStyle,DomClass,Lang){ 
				return Declare("resizeMoveArea", [Evented],{
					//resizeMoveArea - This component allows an easy resize and move of a screen area
					//    1- present handles - 
					//    2- the user may change handles, or move the element dragging it
				...
				//	  : 2- define an event for "mouseenter" to activate moveResizeArea when user enters the area
					...
					activate:function(){
						...
						//
						if(this.oDbg.isDbg("activate")) this.oDbg.display("-------- ENTROU EM ACTIVATE() --------");
						...
						if(!this.objResizeWidget){//ResizeWidget is created and its coordinator property is set to resizeMoveArea (this class)
							...
							var thiz=this;
							if(this.oDbg.isDbg("activate")) this.oDbg.display("objResizeWidget not existing ->creates it and sets resizeMoveArea as coordinator");
							...
							this.endResizeHandler=this.objResizeWidget.on("endResize",Lang.hitch(this,function(e){
								...
								//console.log(this.z1+"--->ResizeWidget -callback de ENDRESIZE em resizeMoveArea.activate() left="+this.position.x+" top="+this.position.y+" width="+this.position.w+" height="+this.position.h);
								if(this.oDbg.isDbg("activate")) this.oDbg.display(this.z1+"--->callback de ENDRESIZE em resizeMoveArea.activate() left="+this.position.x+" top="+this.position.y+" width="+this.position.w+" height="+this.position.h);
								if(this.oDbg.isDbg("activate")) this.oDbg.display("--->ResizeWidget -callback de ENDRESIZE em resizeMoveArea.activate() ENVIA PARA afterResizeEnd ->vai emitir evento de saida...");
								//alert("--->ResizeWidget OUTSIDE -->vai para afterResizeEnd()");
						...
						this.oExchanger.setPointer(function(){	
							if(thiz.oDbg.isDbg("activate")) thiz.oDbg.display("-------------------SAI POR MOVE------------------>this is the way out in moveCoord");
					...
					afterMoving:function(){//this code runs when onMouseUp event is triggered in mover class
						...
						if(this.oDbg.isDbg("afterMoving")) this.oDbg.display("New handles position  left="+this.position.x+" top="+this.position.y+" width="+this.position.w+" height="+this.position.h);
