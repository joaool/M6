		define(["dojo",
				 ...
				 "dojo/dnd/move",
				 "dojo/domReady!"], 
		function(dojo,fBuilder,Exchanger,MoveCanvas,MoveCarrier,ResizeWidget,Declare,Event,Evented,Stateful,Lang,domAttr,on,mouse,Ready,query,DomClass,domStyle,domGeom,DomConstruct,ContentPane,Dom,
				Moveable, Mover,Move){
	...
	return Declare("editViewPort",[Evented,Stateful], {
		...
		canvasPaneId:null,//var with "canvasPaneId"+objEditForm.currentFormNumber, ex. canvasPaneId1, canvasPaneId2, etc
		canvas:null, //canvasPane Node
		carrierPane:null,//carrierPane object
		carrierPaneId:null,//var with "carrierPaneId"+objEditForm.currentFormNumber, ex. carrierPaneId1, carrierPaneId2, etc
		...
		modeRun:false, //Class editViewPort - has a Run mode and a design mode when modeRun=false; 
		...
		//constructor:function(args){	//mixes the passed argument into "this"
			...
			this.overWHandlers=[]; //to avoid sharing of array between diferent instances 
			this.outWHandlers=[];//to avoid sharing of array between diferent instances 
			//this.mouseClickOnWHandlers=[];//to avoid sharing of array between diferent instances 
			/*
			...
			*/
			...
			this.canvasPaneId="canvasPaneId"+objEditForm.currentFormNumber;
			//alert(this.canvasPaneId);
			this.canvasPane=this.placeFormInPane(this.objEditForm,this.canvasPaneId,this.viewPortX,this.viewPortY,this.viewPortW,this.viewPortH,1,"dotted","red");
			this.canvas = dojo.byId(this.canvasPaneId); 
			var canvasBox = domGeom.getMarginBox(this.canvasPane.domNode);//CANVAS BOX
			console.log("editViewPort.constructor "+this.canvasPaneId+" box="+canvasBox.l+","+canvasBox.t+","+canvasBox.w+","+canvasBox.h);//junta + 18 para as scrollbars
		...
		//alert(this.carrierPaneId+" xStyle="+xStyle);
			...
			formOverCanvas.placeAt(this.canvasPaneId);  //o form c/os widgets en cima é posto em cima do Canvas
			...
			*/	
			this.refreshEditViewPort();
			//alert("fim do constructor "+this.objEditForm.name);//ok
		},
		refreshEditViewPort:function(){//to update event handlers after adding new elements to fBuilder array
			// ------------------- event setup --------------------------
			// Se montar os handlers com ---> this.mouseDownOnCanvasHandler=on.pausable(this.canvas,"mousedown",this.mouseDownOnCanvas); 
			//		em mouseDownOnCanvas o scope é o scope da chamada (na qual "this" refere-se a mouseDownOnCanvas e não a "editViewPort"...
			//		vamos usar dojo.hitch(this,"mouseDownOnCanvas") para forçar a function mouseDownOnCanvas a reter o scope original do momento da definição...
		...
		mouseDownOnCanvas:function(e){
			...
			if(this.outsideWidget){ //mouse down outside any widget
				...
				if(this.insideWBorder){//border zone 
					...
					alert("mouseDownOnCanvas insideWBorder RESIZE");
						//dojo.style(dojo.byId("canvasPaneId"), "overflow", "hidden");
						dojo.style(dojo.byId(this.canvasPaneId), "overflow", "hidden");
				...
				}else{ //Mouse was pressed inside the interior zone of the widget - insideWidget (interior)
					if(this.objEditForm.tag(this.currentWidget.posInArr)){ //this widget is marked !!! can drag - at the end of drag it will unmark
						console.log("DRAG SIGNAL before move: x="+this.currentWidget.left+"y="+this.currentWidget.top);
						this.suspendWListeners();//vamos desligar o outwidget enquanto estiver a ser feito o load/undload carrier...
						console.log("mouseDownOnCanvas INTERIOR z1");
						...
						console.log("mouseDownOnCanvas INTERIOR z2");
						//Event.stop(e);//para impedir a ocorrencia de out widget !!!!
						//Now we will move the object to the carrier pane
						this.loadCarrier();//all marked widgets will be loaded in carrier pane
						console.log("mouseDownOnCanvas INTERIOR z3");
						//alert("mouseDownOnCanvas JÁ CARREGADO EM CARRIER");

					//};
					}else{ //it occured a mouse down on the Inside-Interior of a widget that was not tagged(=not marked)
						console.log("mouseDownOnCanvas - INSIDE INTERIOR WIDGET NOT YET TAGGED(o que é o mesmo que marked");
						// poderia marcar logo aqui.... mas o code vai para mouseClickOnWidget e é marcado lá
						// Vamos marcar aqui.
						this.mark(this.currentWidget.posInArr);
						this.markedWidgets=true; //at least one is marked		
						console.log("mouseDownOnCanvas - MARCOU WIDGET="+this.currentWidget.posInArr+" marked="+this.markedWidgets);			
					};
					console.log("mouseDownOnCanvas INTERIOR z4");
		...
		loadCarrier:function(){//all marked widgets will be loaded in carrier pane
			//console.log("mouseDownOnCanvas INTERIOR z2.1");			
			var xTotWidgets=this.objEditForm.totObjects();
			//console.log("mouseDownOnCanvas INTERIOR z2.2");	
		...
		unLoadCarrier:function(difX,difY){//to put widgets that are in carrier pane back to canvas with new coordinates
			...
			for(var i=0;i<xTotWidgets;i++){
				...
				alert("unLoadCarrier coordenadas antigas de "+xId+" left="+xLeft+" Top="+xTop);
					xLeft=parseInt(xLeft)+difX;
					xTop=parseInt(xTop)+difY;
				alert("unLoadCarrier coordenadas novas de "+xId+" left="+xLeft+" Top="+xTop);
	...
	//		this.emit("selectwidget",{});//- new on/event system - whenever one widget is clicked the event signal is emitted
			if(this.objEditForm.field(i).type=="button"){
				...
				domStyle.set(xNode.parentNode,"border","3px solid crimson");//marca o button no pai do id					
			}else{
				dojo.style(xNode, "cursor", "move");
				domStyle.set(this.topDijitNode(this.objEditForm.field(i).props.id,this.objEditForm.field(i).type),"border","3px solid crimson");//marca o widget			
			};	
	...
	//				this.emit("unselectwidget",{});//- new on/event system -the widget to be unMarked is the currently selected. Unselect it. 
		...
		afterMovingCanvas:function(){
			var paneX=domStyle.get(dojo.byId(this.canvasPaneId),"left");//gets X do pane
			var paneY=domStyle.get(dojo.byId(this.canvasPaneId),"top");//gets Y do pane
		...
		afterMovingCarrier:function(){
			var paneX=domStyle.get(dojo.byId(this.carrierPaneId),"left");//gets X do pane
			var paneY=domStyle.get(dojo.byId(this.carrierPaneId),"top");//gets Y do pane
			...
			domStyle.set(dijit.byId(this.carrierPaneId).domNode,"left","0px");			
			domStyle.set(dijit.byId(this.carrierPaneId).domNode,"top","0px");		
		...
		overWidget:function(xWOrder,e){//chamado com dojo.parcial(overWidget,xId) 
			...
			console.log("overWidget x->ENTROU em "+this.objEditForm.field(xWOrder).props.id+" h="+widget.props.style.height+" h="+this.currentWidget.height);
			...
			console.log("overWidget xx->ENTROU em "+this.objEditForm.field(xWOrder).props.id+" widget.w="+widgetPosition.w+" widget.h="+widgetPosition.h);
			...
			console.log("overWidget xxx->ENTROU em "+this.objEditForm.field(xWOrder).props.id+" bottom="+this.currentWidget.bottom+" pos.y="+widgetPosition.y+" pos.h="+widgetPosition.h+" adjY="+this.adjustType(this.currentWidget.type,"y")+" adj2Y="+this.adjustType2(this.currentWidget.type,"y"));

			var xScroll = domGeom.docScroll();

		
			console.log("overWidget ENTROU em "+this.objEditForm.field(xWOrder).props.id+" posInArr="+this.currentWidget.posInArr+" h="+this.currentWidget.height+" scrollY="+xScroll.y); //this is the widget element
		...
		/*
			...
			//Event.stop(e);
			console.log("MOUSECLICKONWIDGET ***");
		...
		*/	
		...
		destroy:function(){
			...
			for(var i=0;i<xTotWidgets;i++){ //resume listeners for overWidget and out Widget event
				...
				this.outWHandlers[i].remove();
				//var this.objEditForm.field(i).props.id);
				console.log("editViewPort.destroy ------------------> remove over and out for widget "+i);
				//this.mouseClickOnWHandlers[i]=this.objEditForm.widget(i).on("mouseup",mouseClickOnWidget);
				//this.mouseClickOnWHandlers[i]=this.objEditForm.widget(i).on("mouseup",Lang.hitch(this,"mouseClickOnWidget"));
			};
			this.objEditForm.formObj.destroy(); //detroy forms and all widgets
			this.canvasPane.destroy();
		},
		...
		adjustType2:function(xType,xDirection){//for each type does an adjustment according to the direction (x or y) to width,height
			console.log("adjustType2 "+xType+" dir="+xDirection);
